...
export default class {
  ...
  async addPath(params: {
    ...
    const result = Array.isArray(params.name)
      ? params.name.map((item) =>
          join(params.prefix, basename(item)).replace(/\\/g, "/")
        )
  ...
  async getTemplate() {
    return await sql((db) => db.templates || []);
  }
  async addTemplate(obj) {
    await sql((db) => {
      if (!db.templates) {
        db.templates = [
          {
  ...
  async removeTemplate(obj) {
    await sql((db) => {
      db.templates = db.templates.filter((item) => item.id !== obj.id);
    });
  }
}
