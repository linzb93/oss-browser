...
const { init: tableItemInit, clickPath, getStyle, isPic, download: downloadItem } = useTableItem();
...
const onAddDialogSubmit = () => {
    // Refresh account list and reinitialize
    getUserInfo(String(userInfo.value.id));
};

onMounted(async () => {
    // Get default account or show login
 ...
 * @param {'setting' | 'see-collect' | 'collect' | 'home-page' | 'upload-history' | 'add-account'} cmd - The command to execute
 */
...
// AddDialog control
const { visible: addDialogVisible, show: showAddDialog } = useLogin();
