...
export default class {
    ...
    async addPath(params: { prefix: string; name: string | string[]; type: 'directory' | 'file' }): Promise<void> {
        const { app, config } = await this.getMatches();
        await app.addPath({
            ...params,
            config,
    ...
    async getHistory({ pageSize, pageIndex }: { pageSize: number; pageIndex: number }) {
